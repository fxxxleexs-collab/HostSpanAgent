...
HostSpan 是一个HostSpan 是一个零侵入、有状态的统一执行引擎。它将本地与远端主机在底层彻底拉平，让 AI Agent 能以同一套标准上下文，无缝跨端接管文件。它将本地与远程主机上的文件、进程、终端和任务统一抽象为同一套上下文，使 Agent 无需在远端部署专用服务，就能像操作本地环境一样持续执行、恢复和调试远程任务。本框架支持以下核心能力：
